...
def _migrate_schema(conn: sqlite3.Connection) -> None:
    for ddl in (
        'ALTER TABLE posts ADD COLUMN row_id INTEGER',
...
def post_record(db_path: str, form_id: str, data: Dict[str, Any],
    ...
    with _connect(db_path) as conn:
        ...
        row_id = row[0]
        conn.execute(
            'INSERT INTO posts (record_id, form_id, data, created_at, updated_at, row_id, author) VALUES (?,?,?,?,?,?,?)',
            (record_id, form_id, json.dumps(data, ensure_ascii=False), now, now, row_id, author)
        )
    return record_id
...
def count_records(db_path: str, form_id: str) -> int:
    """Return total number of records for form_id."""
    with _connect(db_path) as conn:
        row = conn.execute(
...
def list_records(db_path: str, form_id: str,
    ...
    """Return records for form_id ordered by created_at desc.

    offset/limit follow SQL semantics. limit=None returns all records.
    """
    with _connect(db_path) as conn:
        if limit is None:
            rows = conn.execute(
                'SELECT record_id, data, created_at, updated_at, row_id, author FROM posts WHERE form_id=? ORDER BY created_at DESC',
                (form_id,)
            ).fetchall()
        else:
            rows = conn.execute(
                'SELECT record_id, data, created_at, updated_at, row_id, author FROM posts WHERE form_id=? ORDER BY created_at DESC LIMIT ? OFFSET ?',
...
def get_record(db_path: str, record_id: str) -> Optional[Dict[str, Any]]:
    ...
    with _connect(db_path) as conn:
        row = conn.execute(
            'SELECT record_id, data, created_at, updated_at, row_id, author FROM posts WHERE record_id=?',
            (record_id,)
        ).fetchone()
    if row is None:
        return None
...
def make_board_info(list_form_path: str) -> Dict[str, Any]:
    """Return board info dict for JS: list_form_path + parsed schema.

    Used by desktop handlers to expose board metadata to the frontend.
    The JS side uses list_form_path to build the runner URL and schema
    to compute iframe dimensions (same formula as runner.html renderForm).
    """
    info: Dict[str, Any] = {'list_form_path': list_form_path}
    try:
        with open(list_form_path, 'r', encoding='utf-8') as f:
            info['schema'] = json.load(f)
    except Exception:
        pass
    return info

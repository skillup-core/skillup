...
""".format(paths=_json.dumps(paths), x=pos.x(), y=pos.y())
        ...
        if self.bridge:
            ...
            def _do_detach_to_new_window(url_key, w_title, desktop_url, w_app_id):
                ...
                try:
                    ...
                    # Map url_key → app_id so focus_app_window() can find the right window.
                    if not hasattr(engine_ref, '_window_app'):
                        engine_ref._window_app = {}
                    if w_app_id:
                        engine_ref._window_app[url_key] = w_app_id

                    # When the detached window closes, clear the page ref and notify app.
                    _orig_close = new_view.__class__.closeEvent
                    if not hasattr(engine_ref, '_on_detached_close_cbs'):
                        engine_ref._on_detached_close_cbs = {}

                    def _detached_close(self_view, event, _uk=url_key):
                        engine_ref._detached_pages.pop(_uk, None)
                        engine_ref._window_app.pop(_uk, None) if hasattr(engine_ref, '_window_app') else None
                        self._popup_windows.pop(_uk, None)
                        # Clear legacy ref only if it points to this page
                        if engine_ref._detached_page is old_page:
                            engine_ref._detached_page = None
                        cbs = getattr(engine_ref, '_on_detached_close_cbs', {})
                        cb = cbs.pop(_uk, None)
                        if cb:
                            try:
                                cb()
    ...
    def detach_to_new_window(self, url: str, title: str, desktop_url: str, multi: bool = False, app_id: str = ''):
        ...
        Args:
            ...
            app_id: App identifier to associate with this detached window.
        """
        from urllib.parse import urlparse
        parsed = urlparse(url)
        if multi:
            ...
            url_key = parsed.scheme + '://' + parsed.netloc + parsed.path
        if self.bridge:
            self.bridge.detachToNewWindow.emit(url_key, title, desktop_url, app_id)
        else:

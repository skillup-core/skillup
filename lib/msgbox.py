...
def _send_notification(method, params=None):
    ...
    Serializes stdout writes via a process-shared lock kept on `sys` so that
    notifications from this module never interleave with JSON-RPC responses
    written by comm.py. Both modules reach the same Lock instance regardless
    of how each was imported.
    ...
    lock = sys.__dict__.setdefault('_skillup_stdout_lock', threading.Lock())
    with lock:

...
(function() {
     ...
     * In iframe mode, delegates to parent.showToast so all toasts appear
     * in the same desktop-level corner. Falls back to local rendering
     * (standalone Qt window or web browser) when there is no parent.
     *
     * @param {string} msg - Already-translated message text
     * @param {object} [options]
     * @param {string} [options.type] - 'ok' (default) or 'err'
     * @param {number} [options.delay] - Auto-dismiss delay in ms (default 5000)
     */
    window.showToast = function(msg, options) {
        if (window.isInIframe && window.parent && window.parent !== window &&
                typeof window.parent.showToast === 'function') {
            window.parent.showToast(msg, options);
            return;
        }
        // Local fallback: render toast in current document
        const opts = options || {};
        const isErr = opts.type === 'err';
        const delay = opts.delay || 5000;
        let container = document.getElementById('skillup-toast-container');
        if (!container) {
            container = document.createElement('div');
            container.id = 'skillup-toast-container';
            container.style.cssText = 'position:fixed;bottom:0;right:0;padding:12px;z-index:99999;display:flex;flex-direction:column;align-items:flex-end;pointer-events:none';
        ...
        if (typeof bootstrap !== 'undefined' && bootstrap.Toast) {
            ...
            // No Bootstrap available - simple auto-remove
    ...
    window._showWebFileDialog = function(mode, data = {}) {
        return new Promise((resolve) => {
            ...
            // Start at specified directory, fall back to home
            loadDir(data.startDir || '~');
        });
    };

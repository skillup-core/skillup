...
window.showConfirmDialog = function(title, text, options) {
    return new Promise(function(resolve) {
        ...
        var hideNo = !!(options && options.hideNo);
        noBtn.style.display = hideNo ? 'none' : '';
        ...
        var onKeydown = function(e) {
            if (e.key === 'Enter' || e.key === ' ') {
                ...
                if (!hideNo && document.activeElement === noBtn) onNo(); else onYes();
            } else if (e.key === 'Escape') {
                e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
                if (hideNo) onYes(); else onNo();
            } else if (e.key === 'Tab') {
                e.preventDefault();
                if (!hideNo) {
        ...
        var onBackdrop = function(e) { if (e.target === dialog) { if (hideNo) onYes(); else onNo(); } };

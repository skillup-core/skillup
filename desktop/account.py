...
def get_current_user_id() -> str:
    """Return the current OS user ID."""
    return os.environ.get('USER', 'user')


def enrich_user_info(db_path: str, user_id: str) -> Dict[str, Any]:
    """
    Return display info for a user_id: display_name, avatar_small (base64), avatar_mime.
    Falls back gracefully if the account DB has no record or is unavailable.
    """
    result: Dict[str, Any] = {
    ...
    try:
        acct = get_account(db_path, user_id)
        if acct:
            result['display_name'] = acct.get('name') or user_id
            photo_binary, photo_mime = get_account_photo(db_path, user_id, 'small')
            if photo_binary:
                result['avatar_small'] = base64.b64encode(photo_binary).decode('ascii')
                result['avatar_mime'] = photo_mime or 'image/jpeg'
    except Exception:
        pass
    return result
...
def list_accounts(db_path: str) -> list:
    """
    Return all activated accounts with display info.
    Each item: {id, display_name, avatar_small (base64 str or None), avatar_mime}.
    ...
    try:
        ...
        try:
            rows = conn.execute(
                "SELECT id, name, photo_small, photo_mime "
                "FROM accounts WHERE activated=1 ORDER BY id"
            ).fetchall()
            result = []
            for row in rows:
                avatar_small = None
                if row['photo_small']:
                    avatar_small = base64.b64encode(bytes(row['photo_small'])).decode('ascii')
                result.append({
                    'id': row['id'],
                    'display_name': row['name'] or row['id'],
                    'avatar_small': avatar_small,
                    'avatar_mime': row['photo_mime'] or 'image/jpeg',
                })
            return result
        finally:
            conn.close()

...
class SkillFormApp(BaseApp):
    ...
    DESIGNER_RECENT_FILES_KEY = 'designer.recent_files'
    DESIGNER_RECENT_FILES_MAX = 10
    ...
    def __init__(self, engine: Optional['WebUIEngine'], context: AppContext):
        ...
        self._board_config: Dict[str, Any] = {}
        self._current_user: str = account_mod.get_current_user_id()
        self._account_db_path: str = ''
    ...
    def on_run_desktop_initialize(self) -> int:
        ...
        config = self.load_config({
            'runner.last_schema': '',
            'designer.last_file': '',
            self.DESIGNER_RECENT_FILES_KEY: '[]',
            'general.board_dir': '',
        })
        self._board_config = config
        recent_files = self._load_recent_files_from_config(config)

        account_db_cfg = get_desktop_config('general.account_db', '').strip()
        self._account_db_path = account_db_cfg if account_db_cfg else account_mod.get_default_account_db_path()
        try:
            account_mod.init_db(self._account_db_path)
        except Exception as e:
            print(f'[warn ][SkillForm] Failed to init account DB: {e}', file=sys.stderr)
            ...
            try:
                ...
                for arg in extra_args:
                    if arg.startswith('--skillform-run='):
                        ...
                        auto_run_lang = arg[len('--lang='):]
            except Exception:
                pass
        if not auto_run_lang:
            auto_run_lang = get_desktop_config('general.language', 'en')
        ...
        self.state.update({
            ...
            'designer_recent_files': recent_files,
            'auto_run': auto_run_schema is not None,
            'auto_run_lang': auto_run_lang or '',
        }, notify=False)
        ...
        self.register_handlers({
            ...
            'designer_remove_recent': self._handle_designer_remove_recent,
    ...
    def _handle_runner_load_schema(self, data: dict, language: str) -> dict:
        ...
        # Save last used path, but skip board list forms and board detail navigation.
        is_board_list = any(fd.get('type') == 'board' for fd in schema.get('fields', []))
        is_board_detail = bool(data.get('is_board_detail'))
        if not is_board_list and not is_board_detail:
        ...
        if board_command in ('POST', 'MODIFY', 'DELETE'):
            try:
                ...
                if board_command == 'POST':
                    new_id = board_lib.post_record(db_path, form_id, values, author=self._current_user)
    ...
    def _handle_runner_get_state(self, data: dict, language: str) -> dict:
        ...
        return {
            ...
            'auto_run_lang': self.state.get('auto_run_lang', ''),
    ...
    def _handle_designer_load(self, data: dict, language: str) -> dict:
        ...
        if not os.path.exists(path):
            ...
            recent_files = self._update_designer_recent_files(path)
            self.state.update({
                'designer_schema': {},
                'designer_file': path,
                'designer_recent_files': recent_files,
            }, notify=False)
            return {'success': True, 'schema': {}, 'is_new': True, 'recent_files': recent_files}
        ...
        return {'success': True, 'schema': schema, 'recent_files': recent_files}
    ...
    def _handle_designer_get_state(self, data: dict, language: str) -> dict:
        ...
        return {
            ...
            'recent_files': self.state.get('designer_recent_files', []),
        }

    def _load_recent_files_from_config(self, config: Dict[str, Any]) -> List[str]:
        raw = config.get(self.DESIGNER_RECENT_FILES_KEY, '[]')
        files: List[str] = []
        if isinstance(raw, str):
            try:
                parsed = json.loads(raw)
                if isinstance(parsed, list):
                    files = [str(item) for item in parsed if item]
            except Exception:
                files = []

        last_file = str(config.get('designer.last_file', '') or '').strip()
        if last_file:
            files = [last_file] + files

        deduped: List[str] = []
        seen = set()
        for item in files:
            norm = os.path.expanduser(item).strip()
            if not norm or norm in seen:
                continue
            deduped.append(norm)
            seen.add(norm)
            if len(deduped) >= self.DESIGNER_RECENT_FILES_MAX:
                break
        return deduped

    def _handle_designer_remove_recent(self, data: dict, language: str) -> dict:
        path = str(data.get('path', '') or '').strip()
        norm_path = os.path.expanduser(path)
        recent_files = self._load_recent_files_from_config(self._board_config)
        recent_files = [f for f in recent_files if f != norm_path]
        self._save_designer_config({
            self.DESIGNER_RECENT_FILES_KEY: json.dumps(recent_files, ensure_ascii=False),
        })
        self.state.set('designer_recent_files', recent_files)
        return {'success': True, 'recent_files': recent_files}

    def _update_designer_recent_files(self, path: str) -> List[str]:
        norm_path = os.path.expanduser((path or '').strip())
        recent_files = self._load_recent_files_from_config(self._board_config)
        if norm_path:
            recent_files = [norm_path] + [item for item in recent_files if item != norm_path]
        recent_files = recent_files[:self.DESIGNER_RECENT_FILES_MAX]
        self._save_designer_config({
            'designer.last_file': norm_path,
            self.DESIGNER_RECENT_FILES_KEY: json.dumps(recent_files, ensure_ascii=False),
        })
        return recent_files

    def _save_designer_config(self, updates: Dict[str, Any]):
        for key, value in updates.items():
            self._board_config[key] = value
        self.save_config(self._board_config)
    ...
    def _handle_designer_run_standalone(self, data: dict, language: str) -> dict:
        ...
        schema = data.get('schema')
        lang = data.get('lang') or language or 'en'
        if schema is None:
            return {'success': False, 'error': 'No schema provided'}
        ...
        try:
            self._runner_process = _subprocess.Popen(
                [sys.executable, skillup_py, '--desktop', '--app:skillform',
                 f'--skillform-run={tmp_path}', f'--lang={lang}'],
    ...
    def _handle_designer_prepare_code(self, data: dict, language: str) -> dict:
        ...
        return {
            ...
            'python_bin': executor_sh if has_executor else sys.executable,
    ...
    def _handle_get_account_photo(self, data: dict, language: str) -> dict:
        ...
        photo_bytes, mime = account_mod.get_account_photo(self._account_db_path, user_id, size)
        if photo_bytes:
    ...
    def _enrich_records_author(self, records: list) -> None:
        """Populate @author (display name) from @author_id for each record in-place."""
        cache: Dict[str, str] = {}
        for rec in records:
            user_id = rec.get('@author_id') or ''
            if not user_id:
                rec['@author'] = ''
                continue
            if user_id not in cache:
                info = account_mod.enrich_user_info(self._account_db_path, user_id)
                cache[user_id] = info['display_name']
            rec['@author'] = cache[user_id]
    ...
    def _handle_board_read_detail_fields(self, data: dict, language: str) -> dict:
        ...
        try:
            with open(resolved, 'r', encoding='utf-8') as f:
                schema = json.load(f)
            def _field_entry(fd):
                entry = {'id': fd['id'], 'label': fd.get('label', fd['id'])}
                if 'options' in fd:
                    entry['options'] = fd['options']
                for k, v in fd.items():
                    if k.startswith('label.') or k.startswith('options.') or k.startswith('default.'):
                        entry[k] = v
                return entry
            fields = [
                _field_entry(fd)

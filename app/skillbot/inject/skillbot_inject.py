    ...
    try:
        ...
        for wid in prop.value:
            try:
                w = d.create_resource_object('window', wid)
                name = w.get_wm_name()
                if name and 'CDS.log' in str(name) and 'virtuoso' in str(name).lower():
                    pid = 0
                    p = w.get_full_property(pid_atom, Xatom.CARDINAL)
                    if p:
                        pid = p.value[0]
...
def _inject_text_to_ciw(text, log_file="", window_id=None):
    ...
    def _xdt(*args):
        r = _sp.run([_XDOTOOL] + list(args), env=env,
                    capture_output=True, timeout=5)
        if r.returncode != 0:
            _log(f"xdotool {' '.join(args)} rc={r.returncode} stderr={r.stderr.decode(errors='replace').strip()}", "warn")

    # Clear any early paste_done set by a clipboard manager (klipper, xclipboard,
    # etc.) that requests the selection the moment ownership changes — before ctrl+v
    # is sent to CIW.  Cleared here so only CIW's SelectionRequest satisfies the wait.
    if paste_done.is_set():
        _log("paste_done fired before ctrl+v (clipboard manager) — clearing")
        paste_done.clear()
    ...
    if not paste_done.wait(timeout=2.0):
        ...
        _log(f"paste_done in {_time.time() - paste_t0:.3f}s")

    # Send Return directly to CIW window (--window bypasses focus state).
    # Virtuoso can steal focus between windowfocus and paste_done (e.g. on
    # breakpoint hit), so targeting the window ID directly avoids the race.
    r = _sp.run([_XDOTOOL, 'key', '--clearmodifiers', '--window', str(ciw_win.id), 'Return'],
                env=env, capture_output=True, timeout=5)
    if r.returncode != 0:
        _log(f"xdotool key Return rc={r.returncode} stderr={r.stderr.decode(errors='replace').strip()}", "warn")
    _time.sleep(0.05)
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def _handle_debug_command(self, req):
        ...
        if cmd in ("continue", "next"):
            # If bp_code exists, combine it with continue() in a single inject so
            # both execute atomically under one Enter key — avoids the two-inject
            # race where the first Enter can fail and bp_done times out.
            ...
            if bp_code:
                combined = bp_code + '\ncontinue()\n'
                skill_call = _write_and_load(combined, "continue")
            else:
                skill_call = "continue()"

...
document.addEventListener('DOMContentLoaded', function() {
    ...
    editor = CodeMirror.fromTextArea(textarea, {
        ...
        extraKeys: {
            ...
            "Alt-Left": function(cm) { SBNav.goBack(); return false; },
            "Alt-Right": function(cm) { SBNav.goForward(); return false; },
            "Tab": function(cm) {
    ...
    function startPolling() {
        callPython('get_skillbot_config', {}).then(res => {
            ...
            if (res && res.pending_open_dir) {
                _openDirInSkillbot(res.pending_open_dir);
            }
        }).catch(() => {
            refreshStatus();
    ...
    async function _openDirInSkillbot(dirPath) {
        // Set explorer root
        await callPython('file_set_root', { path: dirPath }).catch(() => {});
        EXP.reloadRoot(dirPath);

        // Close all existing tabs, then open root-level files
        const existing = EXP.getTabs().slice();
        ...
        let files = [];
        try {
            const listRes = await callPython('file_list_dir', { path: dirPath });
...
function _applyEditorFontSize(size) {
    if (typeof editor !== 'undefined' && editor) {
        ...
        editor.refresh();
        const wrap = document.querySelector('.sb-editor-wrap');
...
const EXP = (() => {
    ...
    const _iconThemeBase = '/common/resource/fileicon';
...
const SBNav = (function() {
    ...
    function push(pos) {
        if (!pos) return;
        const last = _back[_back.length - 1];
        if (last && last.line === pos.line && last.ch === pos.ch) return;
        _back.push({ line: pos.line, ch: pos.ch });
        if (_back.length > MAX) _back.shift();
        _fwd = [];
    }

    function goBack() {
        if (!editor || _back.length === 0) return;
        const cur = editor.getCursor();
        _fwd.push({ line: cur.line, ch: cur.ch });
        const pos = _back.pop();
    ...
    function goForward() {
        if (!editor || _fwd.length === 0) return;
        const cur = editor.getCursor();
        _back.push({ line: cur.line, ch: cur.ch });
        const pos = _fwd.pop();
        editor.setCursor(pos);
        editor.scrollIntoView(pos, 80);
        editor.focus();
...
const SBFind = (function() {
    ...
    function findNext(reverse) {
        if (!_dialogOpen && editor) {
            // Dialog is closed: if there's a selection (single-line), use it as the query
        ...
        if (!_dialogOpen) {
            ...
            SBNav.push(editor ? editor.getCursor() : null);
            // Use selection anchor (from) so current highlight doesn't block navigation
            const curFrom = editor ? editor.indexFromPos(editor.getCursor('from')) : 0;
            _buildMatches(_lastQuery, _lastCase, _lastRegex);
            if (_matches.length === 0) { _updateStatus(); return; }
            // Forward: first match strictly after curFrom; wrap to 0 if none found
            // Reverse: last match strictly before curFrom; wrap to last if none found
            let best = -1;
            if (!reverse) {
                for (let i = 0; i < _matches.length; i++) {
                    if (editor.indexFromPos(_matches[i].from) > curFrom) { best = i; break; }
                }
                if (best === -1) best = 0;
            } else {
                ...
                if (best === -1) best = _matches.length - 1;
            }
            _matchIdx = best;
        } else {
